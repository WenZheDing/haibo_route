    ...
    namespace Lookup
    {
                ...
                for (int Y = 0; Y < width; ++Y)
                {
                    ...
                    for (int h0 = 0; h0 < headings; ++h0)
                    {
                        ...
                        for (int h1 = 0; h1 < headings; ++h1)
                        {
                            ...
                            reedsshepp_init(start, goal, Constants::r, &path);
                            lookup[X * headings * headings * width + Y * headings * headings + h0 * headings + h1] = reedsshepp_path_length(&path);
        ...
        static void collisionLookup(Constants::config *lookup)
        {
            ...
            for (int q = 0; q < positions; ++q)
            {
                ...
                // p[0].x = c.x - vehicle_length / 2 / cSize;
                // p[0].y = c.y - vehicle_width / 2 / cSize;

                // p[1].x = c.x - vehicle_length / 2 / cSize;
                // p[1].y = c.y + vehicle_width / 2 / cSize;

                // p[2].x = c.x + vehicle_length / 2 / cSize;
                // p[2].y = c.y + vehicle_width / 2 / cSize;

                // p[3].x = c.x + vehicle_length / 2 / cSize;
                // p[3].y = c.y - vehicle_width / 2 / cSize;
                ...
                for (int o = 0; o < Constants::headings; ++o)
                {
                    ...
                    for (int k = 0; k < 4; ++k)
                    {
                        ...
                        // if (stepX > 0)
                        // {
                        //   tMaxX = tDeltaX * (1 - std::fabs(start.x - (long)start.x));
                        // }
                        // else
                        // {
                        //   tMaxX = tDeltaX * std::fabs(start.x - (long)start.x);
                        // }

                        // if (stepY > 0)
                        // {
                        //   tMaxY = tDeltaY *(1 -  std::fabs(start.y - (long)start.y));
                        // }
                        // else
                        // {
                        //   tMaxY = tDeltaY * std::fabs(start.y - (long)start.y);
                        // }
                        if ((stepX > 0 && start.x > 0) || (stepX < 0 && start.x < 0))
                        {
                        ...
                        if ((stepY > 0 && start.y > 0) || (stepY < 0 && start.y < 0))
                        {
                    ...
                    for (int i = 0; i < size; ++i)
                    {
                        for (int j = 0; j < size; ++j)
                        {
                            if (cSpace[i * size + j])
                            {
                                ...
                                lookup[q * Constants::headings + o].pos[count3].x = j - (int)c.x;
                                lookup[q * Constants::headings + o].pos[count3].y = i - (int)c.y;
                    ...
                    lookup[q * Constants::headings + o].length = count3;
